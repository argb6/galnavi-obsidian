...
> [!info] 概述
> `docs/data.json` 是 GitHub 仓库中的数据文件，是线上 D1 数据库（[[02-网站架构/API 端点清单|/nav/api/nav]]）的**滞后镜像**。字段结构一致，但条目数可能不同步。
...
> ⚠️ 两者**不完全同步**：D1 比仓库多 3 条网站（琉璃神社、绅士之庭、萌心次元）。说明 D1 是权威源，仓库 data.json 是定期导出的滞后副本。详见 [[GitHub 仓库的角色]] 的"数据源/镜像"部分。
...
- 被 [[02-网站架构/API 端点清单|/nav/api/featured]] 的 `item_keys` 引用
...
> 当前只有 `simulators` 和 `websites` 有数据，`tools`/`company`/`hanhua` 是规划中的分类（导航栏已预留视图，见 [[02-网站架构/路由与页面体系]]）。

映射关系见 [[03-部署的JS/数据预加载脚本（D1 载入）]] 的 `CAT_MAP`。
...
- 部分用站点自身图标
- 图标加载失败时显示默认 🔗（见 [[03-部署的JS/主应用逻辑脚本（卡片与交互）]] 的 buildCard）
...
- 分类详情 → [[收录站点分类（模拟器与网站）]]
- 标签语义 → [[标签体系]]
- 数据加载 JS → [[03-部署的JS/数据预加载脚本（D1 载入）]]
- 存储 → [[02-网站架构/存储层 D1 与 KV]]
- 上一级 → [[00 知识库地图 (MOC)]]
