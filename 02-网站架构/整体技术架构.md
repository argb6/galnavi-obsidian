...
- **形态**：多个独立 Worker 文件，各司其职（源码 `galnavi/*.js` 确认）：
  - `galnavi.js` — 入口页（永久发布页）
  - `websearch.js` — 主站 + 全部 API（最大，含 D1/KV 查询）
  - `detail.js` — 详情页（查 D1 md_content）
  - `help.js` — 帮助页
  - `about.js` — 关于页
  - `moho.js` — Clash 客户端导航 & Plan 页
- **绑定**：D1（站点业务表）+ 两个独立 KV namespace（轮播图、推荐）
...
### 3. 后端 / 数据
- **D1**（Cloudflare SQLite 数据库）：存放站点核心数据 + 详情页 Markdown 内容（`md_content` 字段）（详见 [存储层 D1 与 KV](存储层 D1 与 KV.md)）
- **KV**（两个独立 namespace）：一个存轮播图、一个存站长推荐（低频变更、需快速读取）

### 4. 安全
- 严格 CSP（详见 [内容安全策略 CSP](../06-安全与SEO/内容安全策略 CSP.md)）
...
- 路由细节 → [路由与页面体系](路由与页面体系.md)
- API 清单 → [API 端点清单](API 端点清单.md)
- 存储分工 → [存储层 D1 与 KV](存储层 D1 与 KV.md)
- 渲染流程 → [请求与渲染流程](请求与渲染流程.md)
- JS 实现 → [内联 JS 总览与加载策略](../03-部署的JS/内联 JS 总览与加载策略.md)
