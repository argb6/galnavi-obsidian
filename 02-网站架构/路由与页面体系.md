...
| `/nav/group/`       | `group.js`     | 神魔殿堂              | 神器、魔器、仙器总览（独立 D1 绑定）     |
...
收录 4 个页面（`/`、`/nav/`、`/nav/help/`、`/nav/group/`），`/nav/about/` 未收录（因当前是维护页）。`/nav/group/`（神魔殿堂）为独立功能页面，使用独立的 D1 绑定。
...
graph LR
    ...
    GROUP -->|D1查询(独立绑定)| RES["resources表<br/>神器/魔器/仙器"]
```
