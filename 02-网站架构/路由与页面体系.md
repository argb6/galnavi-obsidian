...
### 详情视图
点击卡片"介绍详情"会展开该站点介绍（同页内展开，非独立路由）。外链点击触发跳转确认层（见 [外链跳转脚本（Redirect 倒计时）](../03-部署的JS/外链跳转脚本（Redirect 倒计时）.md)）。
...
- 各页面详解 → [入口页（永久发布页）](../05-页面详解/入口页（永久发布页）.md) 等
- 路由背后的 JS → [主应用逻辑脚本（卡片与交互）](../03-部署的JS/主应用逻辑脚本（卡片与交互）.md)
- API 路由 → [API 端点清单](API 端点清单.md)
