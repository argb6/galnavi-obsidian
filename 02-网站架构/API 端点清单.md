...
### 客户端格式化
前端拿到后做映射（见 [[03-部署的JS/数据预加载脚本（D1 载入）]]）：
```javascript
const CAT_MAP = {
...
> 该端点返回线上 D1 数据（29 条），与 GitHub 仓库 `docs/data.json`（26 条）**字段结构一致但条目滞后**——D1 比仓库多 3 条网站。说明 D1 是权威源，仓库 data.json 是滞后镜像。详见 [[04-数据与资源/data.json 数据结构]]。
...
详见 [[03-部署的JS/轮播图脚本（Hero Carousel）]]。
...
详见 [[03-部署的JS/主应用逻辑脚本（卡片与交互）]] 的 featured 部分。
...
- 存储分工 → [[存储层 D1 与 KV]]
- 调用方 JS → [[03-部署的JS/数据预加载脚本（D1 载入）]]
- 数据结构 → [[04-数据与资源/data.json 数据结构]]
- 上一级 → [[00 知识库地图 (MOC)]]
