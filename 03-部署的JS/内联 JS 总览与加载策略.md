...
详见 [数据预加载脚本（D1 载入）](数据预加载脚本（D1 载入）.md)、[主应用逻辑脚本（卡片与交互）](主应用逻辑脚本（卡片与交互）.md)。
...
主站 `/nav/` 的 Content-Security-Policy（详见 [内容安全策略 CSP](../06-安全与SEO/内容安全策略 CSP.md)）：
...
1. [数据预加载脚本（D1 载入）](数据预加载脚本（D1 载入）.md) — 行1
2. [主应用逻辑脚本（卡片与交互）](主应用逻辑脚本（卡片与交互）.md) — 行948（最大）
3. [轮播图脚本（Hero Carousel）](轮播图脚本（Hero Carousel）.md) — 主站
4. [外链跳转脚本（Redirect 倒计时）](外链跳转脚本（Redirect 倒计时）.md) — 主站
5. [入口页发布页弹窗脚本](入口页发布页弹窗脚本.md) — 入口页
6. [帮助页侧栏与锚点脚本](帮助页侧栏与锚点脚本.md) — 帮助页
7. [XSS 防护与 escapeHtml](XSS 防护与 escapeHtml.md) — 通用

## 相关笔记

- 架构背景 → [整体技术架构](../02-网站架构/整体技术架构.md)
- 渲染流程 → [请求与渲染流程](../02-网站架构/请求与渲染流程.md)
- CSP 详解 → [内容安全策略 CSP](../06-安全与SEO/内容安全策略 CSP.md)
