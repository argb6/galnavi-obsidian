...
详见 [[03-部署的JS/数据预加载脚本（D1 载入）]]、[[03-部署的JS/主应用逻辑脚本（卡片与交互）]]。
...
主站 `/nav/` 的 Content-Security-Policy（详见 [[06-安全与SEO/内容安全策略 CSP]]）：
...
行948 脚本（23KB）定义的核心函数（详见 [[03-部署的JS/主应用逻辑脚本（卡片与交互）]]）：
...
1. [[03-部署的JS/数据预加载脚本（D1 载入）]] — 行1
2. [[03-部署的JS/主应用逻辑脚本（卡片与交互）]] — 行948（最大）
3. [[03-部署的JS/轮播图脚本（Hero Carousel）]] — 主站
4. [[03-部署的JS/外链跳转脚本（Redirect 倒计时）]] — 主站
5. [[03-部署的JS/入口页发布页弹窗脚本]] — 入口页
6. [[03-部署的JS/帮助页侧栏与锚点脚本]] — 帮助页
7. [[03-部署的JS/XSS 防护与 escapeHtml]] — 通用

## 相关笔记

- 架构背景 → [[02-网站架构/整体技术架构]]
- 渲染流程 → [[02-网站架构/请求与渲染流程]]
- CSP 详解 → [[06-安全与SEO/内容安全策略 CSP]]
- 上一级 → [[00 知识库地图 (MOC)]]
