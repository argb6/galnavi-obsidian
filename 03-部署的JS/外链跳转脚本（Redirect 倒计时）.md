...
`startRedirect` 不直接绑定到按钮，而是通过 `document` 级的点击委托触发（见 [[03-部署的JS/主应用逻辑脚本（卡片与交互）]]）：
...
> 注意：[[05-页面详解/详情与外链跳转|详情页 `/nav/detail/`]] 是独立 SSR 页面，**没有**这个拦截器，所以详情页的外链（网盘等）是直接 `target="_blank"` 跳转的。
...
- 调用方 → [[03-部署的JS/主应用逻辑脚本（卡片与交互）]]（全局事件委托）
- 详情页 → [[05-页面详解/详情与外链跳转]]
- 安全设计总览 → [[06-安全与SEO/外链安全设计]]
