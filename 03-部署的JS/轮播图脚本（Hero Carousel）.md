...
> 当前线上返回的是 debug 格式（含 debug 字段，KV 中存的是字符串而非 JSON 数组，Worker 解析出错后回退），详见 [[02-网站架构/API 端点清单]]。
...
```
这些图片存放在仓库 `docs/hero/` 目录（详见 [[04-数据与资源/图片素材资源]]），验证了 GitHub 仓库作为静态资源 CDN 的角色。
...
- 数据源 → [[02-网站架构/API 端点清单]]（/nav/api/hero）
- 存储 → [[02-网站架构/存储层 D1 与 KV]]（KV）
- 图片素材 → [[04-数据与资源/图片素材资源]]
- 主应用 → [[03-部署的JS/主应用逻辑脚本（卡片与交互）]]
