...
**要点**：
- **所有用户/D1 数据经 `escapeHtml` 转义**（防 XSS，见 [[03-部署的JS/XSS 防护与 escapeHtml]]）
- 标签限 7 个，避免过长
- 图标智能判断：URL→img，文本→emoji span，空→🔗
- 搜索高亮：用 `escapeRegExp` 转义关键词后正则替换包裹 `<span class="search-highlight">`
- **"介绍详情"指向 `/nav/detail/?item_key=xxx`**（详情页，见 [[05-页面详解/详情与外链跳转]]）
...
**自愈**：fallback 计算后 POST 回 `/nav/api/featured` 写入 KV，后续请求直接命中（详见 [[02-网站架构/存储层 D1 与 KV]]）。
...
- 命中外链 → `startRedirect`（详见 [[03-部署的JS/外链跳转脚本（Redirect 倒计时）]]）
...
- 数据来源 → [[03-部署的JS/数据预加载脚本（D1 载入）]]
- 轮播 → [[03-部署的JS/轮播图脚本（Hero Carousel）]]
- 跳转 → [[03-部署的JS/外链跳转脚本（Redirect 倒计时）]]
- XSS → [[03-部署的JS/XSS 防护与 escapeHtml]]
- 详情页 → [[05-页面详解/详情与外链跳转]]
