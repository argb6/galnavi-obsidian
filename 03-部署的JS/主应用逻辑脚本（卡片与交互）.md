...
> [!info] 位置
> 主站 `/nav/` HTML 的**行 948**，约 23KB，是 GalNavi 最大的脚本块。负责所有客户端交互：初始化、路由、卡片渲染、搜索过滤、标签统计、轮播、外链拦截。
...
function buildCard(item, keyword) {
    ...
    let tagsHtml = item.tags && item.tags.length
        ? '<div class="card-tags">' + item.tags.slice(0, 7)
...
**要点**：
- **所有用户/D1 数据经 `escapeHtml` 转义**（防 XSS，见 [XSS 防护与 escapeHtml](XSS 防护与 escapeHtml.md)）
- 标签限 7 个，避免过长
...
async function renderHomePage() {
        ...
        if (resp.ok) {
            ...
            if (keys.length > 0) {
                ...
                featured = allItems.filter(item => keySet.has(item.id))
                    .map(...).slice(0, 6);
                // 第二层：item_key 无匹配 → 按 title 大小写不敏感匹配
    ...
    // 第三层 fallback：KV 为空 → 取标签含 '推荐' 的前 6 条
    if (!featuredFromKV) {
        featured = allItems.filter(item => item.tags.includes('推荐'))
            .slice(0, 6).map(...);
    }

    // 🔑 同步写入 KV（让后续请求可用）—— 自愈机制
    const featuredKeys = featured.map(item => item.id);
    if (featuredKeys.length > 0) {
        fetch('/nav/api/featured', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ item_keys: featuredKeys })
        });
    }
    // ...渲染推荐区 + 最近更新区
}
...
2. KV `item_keys` 按 `title` 模糊匹配（大小写不敏感）
3. KV 空 → 本地标签匹配（含"推荐"标签）

**自愈**：fallback 计算后 POST 回 `/nav/api/featured` 写入 KV，后续请求直接命中（详见 [存储层 D1 与 KV](../02-网站架构/存储层 D1 与 KV.md)）。
...
- 命中外链 → `startRedirect`（详见 [外链跳转脚本（Redirect 倒计时）](外链跳转脚本（Redirect 倒计时）.md)）
...
- 数据来源 → [数据预加载脚本（D1 载入）](数据预加载脚本（D1 载入）.md)
- 轮播 → [轮播图脚本（Hero Carousel）](轮播图脚本（Hero Carousel）.md)
- 跳转 → [外链跳转脚本（Redirect 倒计时）](外链跳转脚本（Redirect 倒计时）.md)
- XSS → [XSS 防护与 escapeHtml](XSS 防护与 escapeHtml.md)
