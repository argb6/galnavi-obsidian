...
- ❌ 不放广告（"纯净无广"是核心卖点）
- ❌ GitHub 仓库不存放网站源代码（详见 [[04-数据与资源/GitHub 仓库的角色]]）

## 收录规模（截至 2026-07-08）

- 共 **29** 条目（来自 [[04-数据与资源/data.json 数据结构|D1 数据]]）
  - 模拟器 `simulators`：7 条（Joiplay、Kirikiroid2、Mine、Ryujinx、Tyranor、Winlator、盖世游戏）
  - 网站 `websites`：22 条（资源站、论坛、百科等）
- 分类体系规划 5 类：模拟器 / 站点 / 工具 / 会社 / 汉化组（详见 [[04-数据与资源/收录站点分类（模拟器与网站）]]）
...
- 圆角 18px、深阴影
- 零外部 JS/CSS 依赖（全内联，详见 [[03-部署的JS/内联 JS 总览与加载策略]]）

## 相关笔记

- 为什么做 → [[为什么开发 GalNavi（目的与动机）]]
- 作用与价值 → [[GalNavi 的作用与价值]]
- 技术实现 → [[02-网站架构/整体技术架构]]
- 上一级 → [[00 知识库地图 (MOC)]]
