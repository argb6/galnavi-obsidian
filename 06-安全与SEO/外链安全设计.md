...
```
用事件委托，动态渲染的卡片也生效。详见 [主应用逻辑脚本（卡片与交互）](../03-部署的JS/主应用逻辑脚本（卡片与交互）.md)。
...
```
详情页外链在 SSR 时已生成，故逐个绑定即可。详见 [详情与外链跳转](../05-页面详解/详情与外链跳转.md)。
...
- 主站跳转实现 → [外链跳转脚本（Redirect 倒计时）](../03-部署的JS/外链跳转脚本（Redirect 倒计时）.md)
- 主站拦截器 → [主应用逻辑脚本（卡片与交互）](../03-部署的JS/主应用逻辑脚本（卡片与交互）.md)
- 详情页 → [详情与外链跳转](../05-页面详解/详情与外链跳转.md)
- CSP → [内容安全策略 CSP](内容安全策略 CSP.md)
- 安全响应头 → [安全响应头](安全响应头.md)
